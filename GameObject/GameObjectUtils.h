...
namespace Integrian2D::GOUtils
{
	namespace Detail
	{
		template<typename T>
		__NODISCARD constexpr std::string_view WrappedTypeName()
		{
	...
	template <typename T>
	__NODISCARD constexpr std::string_view ConstexprTypeName()
	{
	...
	__NODISCARD __INLINE constexpr uint32_t ConstexprStringHash(const char* pKey, size_t count)
	{
		uint32_t p = 31;
	...
	template<typename T>
	__NODISCARD constexpr uint32_t GenerateComponentID()
	{
